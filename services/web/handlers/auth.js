...
async function login(context) {
  ...
  const providers = await context.storageApi.getProviders()
...
async function oauthCallback(context, { provider: providerName }) {
  ...
  const providers = await context.storageApi.getProvider(name).catch(err => {
    if (err.status === 404) {
      return null
    }
    throw err
...
async function signupAction(context) {
  ...
  const [err, user] = context.storageApi.signup({
    username,
    email,
    remoteAuth
  }).then(
...
async function tokens(context) {
  ...
  const { objects: tokens } = await context.storageApi.getTokens({for: user})
...
async function handleTokenAction(context) {
  ...
  if (action === 'create') {
    const tokenValue = await context.storageApi.createToken({for: user, description})
